...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'描述', name:'description', width: 210,sortable:true},
            ...
            {title:'操作', name:'' ,width:100, align:'center', renderer: function(val,item,rowIndex){
                var h = "";
	            if(WST.GRANT.CJGL_01 && item['status']>0 && item['isConfig']==1)h += "<a class='btn btn-blue' href='"+WST.U('admin/Addons/toEdit','id='+item['addonId'])+'&p='+WST_CURR_PAGE+"'><i class='fa fa-gear'></i>设置</a> ";
    ...
    mmg = $('.mmg').mmGrid({height: h-85,indexCol: true, cols: cols,method:'POST',
        url: WST.U('admin/addons/pageQuery'), fullWidthRows: true, autoLoad: false,remoteSort: true,sortName:'status',sortStatus:'desc',
        plugins: [
            $('#pg').mmPaginator({})
        ]
    });
