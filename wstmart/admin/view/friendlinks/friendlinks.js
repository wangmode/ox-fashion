  ...
  WST.upload({
      ...
      callback:function(f){
        ...
        if(json.status==1){
          ...
          $('#preview').html('<img src="'+WST.conf.RESOURCE_PATH+'/'+json.savePath+json.thumb+'" height="75" />');
        }else{
     ...
     $.post(WST.U('admin/friendlinks/get'),{id:id},function(data,textStatus){
           ...
           if(json.friendlinkId){
           		...
           		if(json.friendlinkIco!=''){
           			$('#preview').html('<img src="'+WST.conf.RESOURCE_PATH+'/'+json.friendlinkIco+'" height="70px" />');
           		}else{
           			$('#preview').empty();
           		}
