...
class GoodsAppraises extends Model {
     ...
     */
	public function queryByPage($shopId=0){
		$where = [];
     ...
     * @throws \think\exception\DbException
     */
	public function userAppraise($userId){
		$where = [];
		$where['g.goodsStatus'] = 1;
		$where['g.dataFlag'] = 1;
		$where['g.isSale'] = 1;
	...
	public function add($userId){
		...
		if(empty($orders))return shopReturn("无效的订单");
		...
		try{	
			...
			if($rs !==false){
				...
				Db::commit();
				return shopReturn('评价成功',1);
			}else{
				return shopReturn($this->getError(),-1);
			}
		}catch (\Exception $e) {
		    Db::rollback();
	        return shopReturn('评价失败',-1);
	    }
