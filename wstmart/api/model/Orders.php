...
class Orders extends Model{
     ...
     */
	public function submit($addressId,$userId,$orderSrc = 0){
		//$deliverType = ((int)input('post.deliverType')!=0)?1:0;
        $deliverType = 0;//0：快递运输。1：自提
		//发票
		$isInvoice = ((int)input('post.isInvoice')!=0)?1:0;
		$invoiceClient = ($isInvoice==1)?input('post.invoiceClient'):'';
		//$payType = ((int)input('post.payType')!=0)?1:0;//1是货到付款
        $payType = 1;//0：货到付款，1：在线付款
		$isUseScore = (int)input('isUseScore');
		$useScore = (int)input('useScore');
		//检测购物车
        $carts = new Carts();
        $carts = $carts->getCarts($userId,true);
		if(empty($carts['carts']))return shopReturn("下单失败，请选择有效的库存商品");
		if($deliverType==0){// 配送方式为快递，必须有用户地址
			//检测地址是否有效
			$address = Db::name('user_address')->where(['userId'=>$userId,'addressId'=>$addressId,'dataFlag'=>1])->find();
			if(empty($address)){
				return shopReturn("无效的用户地址");
			}
		    $areaIds = [];
	        $areaMaps = [];
		...
		try{
			...
			Db::commit();
			return shopReturn("提交订单成功", 1,$orderunique);
		}catch (\Exception $e) {
            Db::rollback();
            return shopReturn('提交订单失败',-1);
        }
	}
     ...
     * @throws \think\exception\DbException
     */
	public function getByUnique($userId){
     ...
     * @param $userId
     * @param $orderStatus
     * @param int $isAppraise
     * @return array
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
	public function userOrdersByPage($userId,$orderStatus='', $isAppraise = -1){
		...
		$page = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		             ...
		             ->field('o.orderRemarks,o.noticeDeliver,o.orderId,o.orderNo,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
	    ...
	    if(count($page['data'])>0){
	    	 ...
	    	 foreach ($goods as $v){
	    	 	 ...
	    	 	 //modified by wangmode 2019.5.27
                 $v['goodsImg'] = formatUrl($v['goodsImg']);
	...
	public function reject($userId){
		...
		$reasonData = WSTDatas('ORDER_REJECT',$reason_id);
		if(empty($reasonData))return shopReturn("无效的拒收原因");
		if($reason_id==10000 && $content=='')return shopReturn("请输入拒收原因");
		if(!empty($order)){
			Db::startTrans();
		    try{
				$data = ['orderStatus'=>-3,'rejectReason'=>$reason_id];
				if($reason_id==10000)$data['rejectOtherReason'] = $content;
     ...
     * @throws \think\exception\DbException
     */
	public function getMoneyByOrder($orderId){
     ...
     * @throws \think\exception\DbException
     */
	public function getByView($orderId, $userId=0){
		...
		if(empty($orders))return shopReturn("无效的订单信息");
		// 获取店铺地址
     ...
     * @throws \think\exception\DbException
     */
	public function getOrderInfoAndAppr($userId,$orderId){
		...
		return ['count'=>$count,'data'=>$goodsInfo,'already'=>$already];
	...
	* 用户-提醒发货
	*/
	public function noticeDeliver($userId){
		...
		try{
			...
			Db::commit();
			return shopReturn('提醒成功',1);
		}catch(\Exception $e){
			Db::rollback();
		}
		return shopReturn('提醒失败',-1);
	}
    ...
    public function waitDeliverById(){
        ...
        $order = Db::name('orders')->field('deliverType,userAddress')->where('orderId','=',$orderId)->find();
