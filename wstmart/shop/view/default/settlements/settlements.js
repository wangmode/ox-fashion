...
function settlement(){
    ...
    for(var s=0;s<rows.length;s++){
        ids.push(rows[s]['orderId']);
    }
	var load = WST.load({msg:'正在提交申请，请稍后...'});
	WST.confirm({content:'您确定要申请结算这些订单吗？',yes:function(){
        $.post(WST.U('shop/settlements/settlement'),{ids:ids.join(',')},function(data,textStatus){
			layer.close(load);
			var json = WST.toJson(data);
		    if(json.status==1){
	            WST.msg(json.msg,{icon:1},function(){
	            	getUnSettledOrderPage(WST_CURR_PAGE);
	            });
		    }else{
		    	WST.msg(json.msg);
		    }
...
function getUnSettledOrderPage(p){
    ...
    mmg2 = $('.mmg2').mmGrid({height: h-121,indexCol: true, cols: cols,method:'POST',
        url: WST.U('shop/settlements/pageUnSettledQuery'), fullWidthRows: true, autoLoad: false,checkCol:true,multiSelect:true,
        plugins: [
            $('#pg2').mmPaginator({})
        ]
    });
