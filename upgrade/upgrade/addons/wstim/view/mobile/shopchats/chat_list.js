...
var showCurrVisit = function(userId){
	...
	var url = WST.U('mobile/goods/detail',{goodsId:goodsInfo.goodsId});
...
var userFunc = {
			...
			for(var i in data.data){
				...
				if(_obj.list.length>1){
					...
					for(var j=0;j<goodsCount;++j){
						...
						var _url = WST.U('mobile/goods/detail',{id:_gObj.goodsId});
	...
	getHistory:function(){
		...
		$.post(url,params,function(data){
			...
			for(var i in data){
				var _obj = data[i];
				var _url = WST.U('mobile/goods/detail',{id:_obj.goodsId});
...
dealContent=function(msg){
	if(isJSON(msg.content)){
		...
		switch(_json.type){
			case 'goods':
				msg.content = '<a href="'+WST.U('mobile/goods/detail',{goodsId:_json.goodsId})+'" target="_blank">'+_json.content+'</a>'
			break;
			case 'orders':
				msg.content = '<a href="'+WST.U('mobile/orders/index',{orderId:_json.orderId})+'" target="_blank">'+_json.content+'</a>'
			break;
			case 'image':
