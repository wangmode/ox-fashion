...
class Auctions extends Base{
	...
	public function add($sid=0){
        ...
        $resourceDomain = ($isApp==1)?WSTConf('CONF.resourceDomain'):WSTConf('CONF.resourcePath');
        $data["auctionDesc"] = str_replace($resourceDomain.'/upload/','${DOMAIN}/upload/',$data['auctionDesc']);
		$data['shopId'] = $shopId;
		...
		try{
			...
			if(false !== $result){
	            ...
	            //拍卖描述图片
	            WSTEditorImageRocord(0, $auctionId, '',$data['auctionDesc']);
	            Db::commit();
	...
	public function edit($sid=0){
        ...
        $resourceDomain = ($isApp==1)?WSTConf('CONF.resourceDomain'):WSTConf('CONF.resourcePath');
        $auction->auctionDesc = str_replace($resourceDomain.'/upload/','${DOMAIN}/upload/',$data['auctionDesc']);
        Db::startTrans();
		try{
			$auction->save();
	        //拍卖描述图片
	        $desc = $this->where('auctionId',$grouponId)->value('auctionDesc');
	        WSTEditorImageRocord(0, $grouponId, $desc, $data['auctionDesc']);
	        Db::commit();
